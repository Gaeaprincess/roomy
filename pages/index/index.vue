	...
	<view>
		...
		<view class="top">
			<region-picker @change="bindRegionChange" :value="region">
				<view class="picker">
					{{region[2]}}
					<image class="arrow" src="/static/image/arrow_.png"></image>
				</view>
			</region-picker>
		...
		<view class="shop" v-for="(item, index) in list" :key="index" @tap="regionFun(item)">
			<image class="img2" :src="item.cover"></image>
			<view class="txt">
				<view class="txt1">{{item.name}}</view>
				<view class="txt1">{{item.location}}</view>
				<view class="txt1">营业{{item.opening_hours}}小时</view>
			</view>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			regionFun: function(item) {
				uni.navigateTo({
					url: '/pagesA/shop/shop?id=' + item.id,
